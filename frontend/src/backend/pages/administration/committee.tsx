...
const ManagementCommittee = () => {
    ...
    const [committeeMembers, setCommitteeMembers] = useState<{ active: CommitteeMember[], inactive: CommitteeMember[] }>({
        active: [],
        inactive: []
    });
    ...
    const fetchCommitteeMembers = async () => {
        ...
        try {
            ...
            console.error(err);
            alert('Error fetching committee members');
        } finally {
            setLoading(false);
        }
    };
    ...
    const openModal = (member: CommitteeMember | null = null) => {
        if (member) {
            setFormData({
                id: member.id,
                name: member.name,
    ...
    const displayMembers = (members: CommitteeMember[], page: number) =>
        ...
        <div className="flex flex-col w-full h-full px-6 pt-2 gap-6">
            ...
            >
                <form onSubmit={handleSubmit} className="p-6">
                    <h2 className="text-2xl font-bold mb-4">
                        {isEditing ? 'Edit Management Committee Member' : 'Add Management Committee Member'}
                    </h2>
            ...
            <div className="w-full bg-white p-6 rounded shadow-md">
                <div className='w-full flex justify-between items-center'>
                    <h2 className="text-2xl font-bold mb-4">Management Committee</h2>
                ...
                {loading ? (
                    ...
                    ['active', 'inactive'].map((status) => (
                        <div key={status} className="mb-6">
                            ...
                            {committeeMembers[status].length > 0 ? (
                            ...
                            {committeeMembers[status].length > itemsPerPage && (
                                <ReactPaginate
                                    previousLabel={'«'}
                                    nextLabel={'»'}
                                    breakLabel={'...'}
                                    pageCount={Math.ceil(committeeMembers[status].length / itemsPerPage)}
