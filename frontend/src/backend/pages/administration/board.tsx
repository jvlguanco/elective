...
const BoardOfRegents = () => {
    ...
    const [boardMembers, setBoardMembers] = useState<{ active: BoardMember[], inactive: BoardMember[] }>({
        active: [],
        inactive: []
    });
    ...
    const fetchBoardMembers = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_ROOT}/about/board-of-regents`);
            setBoardMembers({ active: res.data.active, inactive: res.data.inactive });
        } catch (err) {
            console.error(err);
            alert('Error fetching board members');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEditing && formData.id) {
                const res = await axios.put(`${import.meta.env.VITE_API_ROOT}/about/board-of-regents/${formData.id}`, data);
                alert(res.data.message);
            } else {
                const res = await axios.post(`${import.meta.env.VITE_API_ROOT}/about/board-of-regents`, data);
                alert(res.data.message);
            }
            closeModal();
            fetchBoardMembers();
    ...
    const handleDelete = async (id: number) => {
        if (window.confirm('Are you sure you want to delete this member?')) {
            try {
                await axios.delete(`${import.meta.env.VITE_API_ROOT}/about/board-of-regents/${id}`);
    ...
    const displayMembers = (members: BoardMember[], page: number) =>
        ...
        <div className="flex flex-col w-full h-full px-6 pt-2 gap-6">
            ...
            >
                <form onSubmit={handleSubmit} className="p-6">
                    <h2 className="text-2xl font-bold mb-4">
                        {isEditing ? 'Edit Board of Regent Member' : 'Add Board of Regent Member'}
                    </h2>
            ...
            <div className="w-full bg-white p-6 rounded shadow-md">
                <div className='w-full flex justify-between items-center'>
                    <h2 className="text-2xl font-bold mb-4">Board of Regents</h2>
