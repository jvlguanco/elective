...
const BoardOfRegents = () => {
    ...
    const fetchBoardMembers = async () => {
        setLoading(true);
        try {
            const res = await axios.get('http://localhost:5000/about/board-of-regents');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEditing && formData.id) {
                const res = await axios.put(`http://localhost:5000/about/board-of-regents/${formData.id}`, data);
                alert(res.data.message);
            } else {
                const res = await axios.post('http://localhost:5000/about/board-of-regents', data);
                alert(res.data.message);
            }
            closeModal();
            fetchBoardMembers();
    ...
    const handleDelete = async (id: number) => {
        if (window.confirm('Are you sure you want to delete this member?')) {
            try {
                await axios.delete(`http://localhost:5000/about/board-of-regents/${id}`);
